...
const report={status:'EXPERIMENT: one-way near-surface patch centroids, not validated articular landmarks',arms:[],
  ...
  files:[...candidate.files,...[input,'scripts/experiment-wrist-joints.mjs','scripts/lib/surface-containment.mjs'].map(path=>({path,sha256:hash(path)}))]};
for(const arm of candidate.arms){
  const meshes=Object.fromEntries(['scapula','humerus','radius','ulna','scaphoid','lunate'].map(name=>{
  ...
  const scaphoid=contact(meshes.radius,meshes.scaphoid),lunate=contact(meshes.radius,meshes.lunate);
  const wrist=scaphoid.centre.map((v,i)=>(v+lunate.centre[i])/2);
  const row={side:arm.side,shoulder,elbow,elbowPatches:{radius,ulna},wrist,wristPatches:{scaphoid,lunate}};report.arms.push(row);
...
fs.writeFileSync('.cache/arm-registration/wrist-joints.json',JSON.stringify(report,null,2)+'\n');
