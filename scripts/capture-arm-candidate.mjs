...
const candidate=JSON.parse(fs.readFileSync('.cache/arm-registration/candidates.json'));
...
try {
  ...
  await page.screenshot({path:'.cache/arm-registration/before.png'});
  const transformed=await page.evaluate(async ({url,arms})=>{
    ...
    for(const arm of arms)for(const part of arm.parts.filter(p=>p.transformed)){
      ...
      const m=arm.linear,t=arm.translation;
  ...
  assert.equal(new Set(transformed).size,60);
  await banner('미채택 교정 후보 — 실험 전용 / 공개 모델에 적용하지 않음');
  await page.screenshot({path:'.cache/arm-registration/after.png'});
  assert.deepEqual(errors,[]);
  console.log('Captured unchanged baseline and rejected 60-bone candidate in disposable local scene; browser errors 0.');
