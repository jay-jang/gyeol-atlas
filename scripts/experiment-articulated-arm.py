"""Offline two-link candidate; geometric pivots are not validated joints.
...
paths = ['docs/anatomy-alignment/hand-candidates.json', '.cache/arm-registration/joints.json', '.cache/arm-registration/landmarks.json']
...
keep_upper = '--upper-existing' in sys.argv
coupled_hand = '--coupled-hand' in sys.argv
assert not (set(sys.argv[1:]) - {'--upper-existing', '--coupled-hand'}), 'Unknown experiment mode'
assert not coupled_hand or keep_upper, 'Coupled-hand comparison requires existing upper length'
report = {'status': 'EXPERIMENT ONLY: two-link endpoint constraints are not anatomical joint validation',
          'mode': 'existing-humerus-and-coupled-hand' if coupled_hand else 'existing-humerus-length' if keep_upper else 'hand-scale-for-both-links',
          'fitRegion': 'forearm skin only; coupled hand receives the forearm transform without an additional hand fit',
          'limitations': ['Proxy closure is not articular contact or collision validation.',
                          'Uneven original skin vertex density can bias point-to-point fitting.',
...
for hand in hand_report['arms']:
    ...
    source = forearm_cloud('source', side, W, forearm_length, (W - E) / forearm_length)
    target_profile = next(p for p in landmarks['profiles'] if p['body'] == 'target' and p['side'] == side)
    target = forearm_cloud('target', side, target_wrist, forearm_length * scale, np.array(target_profile['axis']))
    tree = cKDTree(target)
    ...
    row = {'side': side, 'parts': parts, 'scaleFromDonor': scale, 'upperScaleFromDonor': upper_scale, 'sourceShoulder': S.tolist(), 'sourceElbow': E.tolist(),
           'sourceWrist': W.tolist(), 'targetShoulder': target_shoulder.tolist(), 'targetElbow': elbow.tolist(),
...
for path in paths + ['scripts/experiment-articulated-arm.py', 'scripts/lib/arm-kinematics.py']:
    files[path] = hashlib.sha256((ROOT / path).read_bytes()).hexdigest()
report['files'] = [{'path': p, 'sha256': h} for p, h in files.items()]
prefix = 'articulated-coupled-hand' if coupled_hand else 'articulated-existing-upper' if keep_upper else 'articulated'
