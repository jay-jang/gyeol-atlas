...
bones.discard('FMA12513') # erroneous/cross-system source relation: eyeball remains an organ
...
if 'FMA12513' in assets: assets['FMA12513']['layer'] = 'organ' # source tree cross-link; eyeball is not skeleton
...
added = []
for layer, ids in [('muscle', muscles), ('nerve', nerves), ('organ', organs), ('vessel', vessels), ('bone', bones | teeth)]:
...
report = {'sourceCommit': inputs['commit'], 'before': dict(before),
          ...
          'requiredSourceIds': {'bone': sorted(bones | teeth), 'muscle': sorted(muscles), 'nerve': sorted(nerves), 'organ': sorted(organs)},
