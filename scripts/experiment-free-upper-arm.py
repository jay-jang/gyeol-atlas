...
positions and forearm length are constrained; humerus scale is fitted inside
explicit bounds. Joint contacts and containment are separate screens.
"""
import hashlib
...
area_samples = True
wrist_contact = True
assert not sys.argv[1:], 'This experiment has no optional modes'
prefix = 'free-upper'
report = {'status': 'EXPERIMENT ONLY: two-link endpoint constraints are not anatomical joint validation',
          ...
          'humerusScaleRange': [.85, 1.15], 'humerusLengthFitted': True,
          ...
          'limitations': ['Proxy closure is not articular contact or collision validation.',
                          ...
                          'Upper twist stays closest to its deployed orientation; humerus scale is fitted within 0.85–1.15 and reachability bounds. Elbow/wrist contacts and finger articulation are not optimized.'],
          'arms': []}
for hand in hand_report['arms']:
    ...
    distance = np.linalg.norm(target_wrist - target_shoulder)
    lower_scale = max(.85, (abs(distance - forearm_length * scale) + 1e-6) / upper_length)
    higher_scale = min(1.15, (distance + forearm_length * scale - 1e-6) / upper_length)
    assert lower_scale < upper_scale < higher_scale, 'Initial upper length outside reachable interval'
    ...
    def transforms(params):
        centre, radius, u, v = kin.elbow_circle(target_shoulder, target_wrist, upper_length * np.exp(params[2]), forearm_length * scale)
    ...
    starts = [np.array([swivel, twist, np.log(upper_scale)]) for swivel in np.arange(-np.pi, np.pi, np.pi / 4) for twist in [-np.pi / 4, 0, np.pi / 4]]
    ...
    for initial in starts:
        ...
        for iteration in range(50):
            ...
            fit = least_squares(residual, params, loss='soft_l1', f_scale=.004, max_nfev=25,
                                bounds=([-2 * np.pi, -np.pi, np.log(lower_scale)], [2 * np.pi, np.pi, np.log(higher_scale)]))
    ...
    upper_scale = float(np.exp(params[2]))
    centre, radius, _, _ = kin.elbow_circle(target_shoulder, target_wrist, upper_length * upper_scale, forearm_length * scale)
...
for path in paths + ['scripts/experiment-free-upper-arm.py', 'scripts/lib/arm-kinematics.py', 'scripts/lib/surface-sampling.py']:
