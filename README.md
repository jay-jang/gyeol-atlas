...
- 남녀의 표시·검색·기관 묶음을 분리합니다. 여성 HRA 전신은 다른 여성 기증자의 하체 근육과 출처를 구분한 남성 유래 보완 골격을 포함한1,220개 구조입니다. 추가로 여성 CT에서 확보한 위·부신과 주변 기관11개를 **별도 상세 보기**로 제공합니다. 식도·등 근육군은 CT 수록 구간만 포함합니다. 서로 다른 신체 자료를 전신에 중첩하지 않으며 상체 근육 전체·일부 말초신경은 여전히 미수록입니다. ‘전체 기관 완비’가 아닙니다. [검증 및 한계](docs/anatomy-alignment/VERIFICATION.md)
- 인체 구조도 중심의 전체 화면과 떠 있는 도구 패널, 모바일 축약 패널, 호버·키보드·터치 조작
- 0.5% 간격 201단계 연속 박리와 7계통(체표·근육·골격·장기·혈관·림프·신경) 빠른 보기. 박리는 좌표 이동 없는 순차 가시화이며 실제 조직 깊이의 측정값은 아닙니다. 여성 경혈 표식은 표면 좌표 검수 전이라 숨깁니다.
- 주요 기관의 독립 상세 보기·세부 구조 선택·전신 복귀. 남성 심장83·간60·폐280개 상세 참조는 BodyParts3D 4.0의 원본 하위 관계를 사용하며 전신과 별도 화면 범위에서 표시합니다. 원본이 단일 메쉬인 기관의 미세구조를 임의로 만들지 않습니다.
