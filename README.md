...
체표→근육→골격→장기→혈관→신경 깊이 슬라이더, 여섯 단계의 단독 보기, 레이어 조합·개별 불투명도, 구조 선택·확대·단독 보기, 앞쪽 메쉬 절개 보기를 제공합니다. 신규 레이어는 표시할 때 불러옵니다. 신경과 혈관 단계에는 기존 개별 검색 구조와 좌우 전신 보완 모델을 함께 표시합니다. WASD/방향키 이동, Q/E 높이, Shift 빠른 이동, 휠 확대·축소 및 터치 이동 버튼을 지원합니다.
