...
test('continuous dissection peels individual muscle meshes and exposes deep systems',async({page})=>{
 ...
 await depth.fill('96');await ready(page);expect((await snapshot(page)).stage).toBe(5);await expect(depth).toHaveAttribute('aria-valuetext','96% 해부 깊이');
