...
- Female HRA assembly: Kristen Browne and Heidi Schlehlein, Human Reference Atlas / HuBMAP, *3D Reference Organ Set for Female*, v1.10, plus eight pelvic structures from v1.5. CC BY 4.0, based on the NLM Visible Human Dataset with the reference-brain exception below. [Reference library](https://humanatlas.io/3d-reference-library).
- Reference brain: the 283 brain meshes in that assembly derive from the Allen reference atlas, not a female donor brain scan. HRA describes mirroring 141 structures and resizing the resulting reference brain for the male and female bodies; brain v1.3 is CC BY 4.0. [HRA's NIH 3D source description](https://3d.nih.gov/entries/3DPX-020959). The original female assembly's brain L/R labels disagree with its eye/bone side convention; original names, IDs and geometry are retained pending correction. See [source comparison](docs/anatomy-alignment/NEURAL_BONE_RELATIONS.md).
...
Upstream adaptations include coordinate conversion, transform baking, mesh simplification, quantized normals, and documented regional fitting of donor muscle and borrowed bones. GYEOL preserves the packed female source files and native female coordinates, assigns display layers, links the exact source organ memberships, hides pregnancy references unless selected, suppresses overlapping donor rectus muscles by default, and labels borrowed bones in a distinct grey color. GYEOL enables donor muscles and borrowed bones in their corresponding layers. No male organs are added to the female body. Female stomach, upper-body muscles and some peripheral nerves are absent from this HRA source; female acupoint coordinates are unvalidated and hidden. This is not a complete or clinically registered atlas.

2026-09-20 partial adaptation: copied runtime geometry for 60 male-derived arm/hand bones receives a static registration recorded in `data/catalog/female-arm-registration.json`. Position and normal transforms reduce gross arm displacement; finger alignment and other donor-body mismatches remain. These bones are not relabelled as native female anatomy. Native HRA parts, independent CT details, and source packed bytes are unchanged. The registered model material, model-derived registration coordinates, and its rendered images retain CC BY 4.0 attribution to BodyParts3D/DBCLS and the stated upstream sources; application code licensing does not replace model licensing. See `docs/anatomy-alignment/FEMALE_ARM_PARTIAL_REGISTRATION.md` for modifications and limitations. No clinical validation or endorsement is implied.
